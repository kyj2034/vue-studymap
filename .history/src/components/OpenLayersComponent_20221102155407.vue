  ...
  methods:{
    ...
    drawPoly(){
      ...
      typeSelect.onchange = function () {
        map.removeInteracction(draw);
        addInteraction();
      };

      document.getElementById('undo').addEventListener('click', function () {
        draw.removeInteracction();
      });

      addInteraction();
    },
    addInteraction() {
        let value = tupeSelect.value;
        if(value !== 'None'){
          let geometryFunction;
          if(value === 'Square') {
            value = 'Circle';
            geometryFunction = createReqularPolygon(4);
          } else if (value === 'Box') {
            value = 'Circle';
            ...
            geometryFunction = function( coordinates, geometry ) {
              ...
              for (let i = 0; i < numPoints; ++i) {
                const angle = rotation + ( i * 2 * Math.PI) / numPointsl
